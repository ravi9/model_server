...
"""Inference client for benchmarks for Triton Inference Server REST"""
...
class KFS_Client_REST(BaseClient):
    """
    ...
    # override
    def get_stub(self):
        self.print_info("creating triton client {self.address}:{self.grpc_port}")
    ...
    def show_server_status(self):
        ...
        client = httpclient.InferenceServerClient(f"{self.address}:{self.rest_port}") ####
    ...
    def get_model_metadata(self, model_name, model_version=None, timeout=60):
        ...
        client = httpclient.InferenceServerClient(f"{self.address}:{self.rest_port}")
        model_metadata = client.get_model_metadata(model_name)  # dont need to use, predict will use ones gathered via grpc
        self.print_info(f"{model_metadata}")
    ...
    def prepare_batch_requests(self):
        ...
        for index in range(self.dataset_length):
            ...
            for input_name, xbatches in self.xdata.items():
                self.print_info("NameXXX", input_name)
                ...
                self.print_info("SHAPE XXX", shape)
                self.print_info("DTYPE XXX", self.inputs[input_name]["dtype"])
            ...
            self.requests.append((batch_length, inputs_array))
