...
- When that parameter is set to a numerical value, it is changing the model batch size at the service start. It accepts also [ranges](./dynamic_shape_dynamic_model.md) and value [auto](./dynamic_bs_auto_reload.md).
...
Each configuration can perform the best in different workloads. For all available options check [dynamic input parameters](./dynamic_input.md) page.
...
*Note:* Some models can accept dynamic dimensions, or dynamic ranges. In that case a tuple could look like e.g. `"(-1,3,200:300,200:300)"`
...
Each dynamic configuration can perform the best in different workloads. For all available options check [dynamic input parameters](./dynamic_input.md) page.
